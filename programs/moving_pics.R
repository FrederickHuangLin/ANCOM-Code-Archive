...
write_csv(res$out, "outputs/moving_pics_basic.csv")
...
fig  
ggsave("images/moving_pics_basic.jpeg", height = 5, width = 6.25, units = "in", dpi = 300)  
...
write_csv(res$out, "outputs/moving_pics_cov_adj.csv")
...
fig = res$fig +  
  ...
  geom_text(data = dat_ann, aes(x = x, y = y, label = label), 
            size = 4, vjust = -0.5, hjust = 0, color = "orange", parse = TRUE)
fig  
ggsave("images/moving_pics_cov_adj.jpeg", height = 5, width = 6.25, units = "in", dpi = 300)  
